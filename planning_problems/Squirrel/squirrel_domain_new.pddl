...
(:durative-action put_object_in_box
	...
	:condition (and
		...
		(at start (holding ?v ?o1))
		(at start (can_fit_inside ?o1 ?b))
)
	:effect (and
		(at start (not (holding ?v ?o1)))
...
(:durative-action unstack_object
	...
	:condition (and
		...
		(at start (can_pickup ?v ?o2)))
	:effect (and
		(at start (not (gripper_empty ?v)))
		(at start (not (on ?o2 ?o1)))
		(at end (not (object_at ?o2 ?wp)))
		(at end (clear ?o1))
		(at end (holding ?v ?o2)))
)
...
(:durative-action stack_object
	...
	:condition (and
		(over all (robot_at ?v ?wp))
		(over all (object_at ?o2 ?wp))
		(at start (clear ?o2))
		(at start (holding ?v ?o1))
		(at start (can_stack_on ?o1 ?o2))
		)
	:effect (and
		(at start (not (clear ?o2)))
		(at start (not (holding ?v ?o1)))
		(at end (gripper_empty ?v))
		(at end (object_at ?o1 ?wp))
		(at end (on ?o1 ?o2)))
)
