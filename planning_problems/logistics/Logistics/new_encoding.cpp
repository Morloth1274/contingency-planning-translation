...
int main(int argc, char **argv)
{
	...
	bool factorise = argc == 7;
	std::cout << "Creating a logistics problem with " << nr_cities << " cities, with " << nr_locations_per_city << " locations, " << nr_airports_per_city << " airports, " << nr_trucks_per_city << " trucks, and " << nr_packages_per_city << " packages per city.";
	...
	{
		airplane = new Airplane("airplane", *cities[0]->airports_[0]);
	}
	...
	if (factorise)
	{
		...
		generateDomain("test_domain.pddl", basis_kb, knowledge_bases, cities, trucks, *airplane, packages, factorise);
		std::cout << "Generate problem..." << std::endl;
		generateProblem("test_problem.pddl", basis_kb, knowledge_bases, cities, trucks, *airplane, packages, factorise);
	}
	else
	{
